...
@Component({
  ...
  styleUrls: ['./pay-for-access-2.component.css']
})

export class PayForAccess2Component implements OnInit {
      ...
      this.userService.getUserType(this.idUser).subscribe(
        (response: any) => {
          if (response.success) {
            ...
            this.router.navigate([`/welcome-user${typeUser}`], { queryParams: { Id_User: this.idUser } });
          } else {
