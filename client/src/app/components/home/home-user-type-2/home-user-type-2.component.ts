...
import { BusinessExpensesService } from '../../../services/business-expenses.service';

@Component({
  selector: 'app-home-user-type-2',
  templateUrl: './home-user-type-2.component.html',
  styleUrls: ['./home-user-type-2.component.css']
})
export class HomeUserType2Component implements OnInit {
  ...
  constructor(
    ...
    private businessExpensesService: BusinessExpensesService,
  ...
  loadExpenses(month: number, year: number) {
    if (this.idUser) {
      this.businessExpensesService.getExpensesByMonth(this.idUser, month, year).subscribe(
        (resp: any) => {
          this.expenses = resp;
        },
        err => console.log(err)
  ...
  viewExpenseDetail(id: string) {
    ...
    this.router.navigate(['/business-expense', id]);
  }

  createNewExpense() {
    if (this.idUser) {
      this.router.navigate(['/business-expense-form'], { queryParams: { Id_User: this.idUser } });
    } else {
      console.error('Usuario no autenticado');
    }
  }
